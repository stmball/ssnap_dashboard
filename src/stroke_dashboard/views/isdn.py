...
def isdn():
    list_of_regions = (
        pd.read_csv("data/overview/by_metric/summary_data_ISDN_SSNAP score.csv")
        .columns[1:]
        .sort_values()
    )

    st.title("SSNAP - ISDN Overview")
    ...
    national = pd.read_csv(
        "data/overview/by_metric/summary_data_ISDN_SSNAP score.csv", index_col=0
    )

    national = national.mean(axis=1)

    region_data = pd.read_csv(f"data/processed/ISDN/{regions}.csv").drop(0)
    ...
    st.header("SSNAP Scores for ISDN")
